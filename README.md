...
**Output:** `synthetic_pvalues.csv` contains individual test p-values;
`synthetic_summary.csv` contains rejection rates and mean p-values by scenario.
Both are written to `results/reproduced_validation/`. The published calibration
summary is in `results/validation/`.
...
**Output:** trajectory length, memory, runtime for each implementation, and
speedup. The slow implementation is skipped at the largest setting, so its
runtime and speedup are recorded as missing there.
...
**Output:** JSON reports in `results/rollout_checks/`. Check for
`all_checks_passed: true` before generating the full experiment. These checks
cover trajectory structure and reproducibility; the short comparison also checks
agreement with the reference loop.
...
**Output:** 300 compressed trajectories under `data/generated/L1000/`,
`L5000/`, and `L10000/`, plus `data/generated/manifest.csv`.
Each NPZ contains `action` and `reward`; the manifest records trajectory-level
estimates, exposure summaries, and generation metadata.
...
The remaining analysis defaults match the reported experiment: input directory
`data/generated`, 2,000 resamples, alpha 0.05, test seed 50000, total-effect memory
5,000, and carryover horizons 0 / 1,000 / 2,500 / 5,000 / 10,000.

The script runs seven tests per trajectory: one total-effect CRT, one anticipation
PIRT, and five carryover CRTs. It also reads the working manifest to summarize
estimator bias, SD, and RMSE, and generates the figures.

New results go to `results/reproduced/`; the published results remain in
